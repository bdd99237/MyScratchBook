...
public struct HSBColor
{
    ...
    {
        HSBColor ret = new HSBColor(0f, 0f, 0f, color.a); //검은색과 매계변수의 알파값

        float r = color.r; //매개변수의 각 색값 추출
        float g = color.g;
        float b = color.b;

        float max = Mathf.Max(r, Mathf.Max(g, b)); //3색중 가장 큰값을 받아온다.

        if (max <= 0)
        {
            return ret; //rgb = 0이므로 검은색이다. 그래서 검은색 리턴해준다.
        }

        float min = Mathf.Min(r, Mathf.Min(g, b)); //3색중 가장 작은 값을 받아온다.
        float dif = max - min;
