    ...
    static Vector3 NearestPoint(Vector3 lineStart, Vector3 lineEnd, Vector3 point)
{
    ...
    static public Vector3 NearestPointStrict(Vector3 lineStart, Vector3 lineEnd, Vector3 point)
{
    Vector3 fullDirection = lineEnd - lineStart; //끝점과 시작점 사이의 거리
    Vector3 lineDirection = Vector3.Normalize(fullDirection); //노멀라이즈
    float closestPoint = Vector3.Dot((point - lineStart), lineDirection) / Vector3.Dot(lineDirection, lineDirection); //가장 가까운점을 구하는 부분.
    return lineStart+(Mathf.Clamp(closestPoint,0.0f,Vector3.Magnitude(fullDirection))*lineDirection); //시작위치 + 끝점을 향한 단위벡터 * 사이거리
}

static public Vector2 NearestPointStrict(Vector2 lineStart, Vector2 lineEnd, Vector2 point)
{
...
static public float GaussFalloff(float distance, float inRadius)
    {
        return Mathf.Clamp01(Mathf.Pow(360.0f, -Mathf.Pow(distance / inRadius, 2.5f) - 0.01f)); //값을 0~1로 제한.
        //예 : 360^(-((√4/2)^2.5)-0.01) = 0.00261899382 (현재 distance를 거리의 제곱근으로 보내줌. 거리가 4, 선의 굵기가 2일때의 상황.)
    }
...
retval =  start+diff;
   ...
   return retval;
    ...
    {
        t = Mathf.Clamp01(t); //Mathf.Clamp01는 0~1사이로 제한하는 기능
        float t2 = 1 - t;
    ...
    static public Vector2 NearestPointOnBezier(Vector2 p, Vector2 p0, Vector2 p1, Vector2 p2, Vector2 p3)
    {
    ...
    static public bool IsNearBezier(Vector2 p, BezierPoint point1, BezierPoint point2, float rad)
    {
        if (point1.curve_2 != point2.curve_1)
        {
            Debug.LogError("Curves Not The Same");
            return false;
        }

        BezierCurve curve = point1.curve_2;
        ...
        float sec = point1.curve_2.aproxLength / 10;
    ...
    static public bool IsNearBeziers(Vector2 p, List<BezierPoint> points, float rad)
    {
